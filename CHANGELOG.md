...
- **Tests** — the suite grew across this branch (now 299 passed + 1 skipped) and is now gated by CI: `scripts/test_notify.py` (transition semantics, payload shapes, atomic state-on-success), `scripts/test_api_health_check.py` (single-session checker, degraded classification, `RALLY_SERVICES` parsing/subset/allowlist, keystone-always), `scripts/test_run_summary.py` (`first_error` contract, `run_state`, seed-from-set), `scripts/test_render_report.py` (payload normalization, percentile/atomic/error aggregation, data-block escaping + breakout resistance, truthy-non-dict shape resilience, empty-state fallback), `scripts/test_backfill_reports.py` (historical-report migration: regenerate old report, idempotency, missing-html, non-run/`branding` dir filtering, malformed-json empty-state), `exporter/test_rally_exporter.py` (API/announcement gauges, fail-closed `rally_api_overall_up`), `dashboard/test_getrunstatus.py` and `dashboard/test_report_js.py` (Node contract tests for the dashboard/report pure helpers), and `dashboard/test_serve.py` (the `runs/` report branch with denial cases and a single-strict-CSP assertion shared by reports and dashboard assets)
...
- **Scripts** — the manual `purge_orphans.sh` now reaps **empty unknown-owner** RGW orphans, not just rally-owned ones. An orphan whose Keystone project is *confirmed gone* (`rgw_find_orphaned_users` emits only authoritative 404s; inconclusive lookups raise `_RGW_LISTING_ERRORS` and abort the purge) and which holds **zero buckets/objects** is now eligible — zero data means zero loss, and Keystone implicit-tenant users self-heal (Ceph recreates them on the next authenticated access). This clears pre-provenance-ledger orphans (RGW users from projects deleted before `rally_project_ids.log` logging began) that the ledger-only gate left stuck as `unknown_owner` forever, making `RallyRgwOrphanedUsers` fire indefinitely. Unknown-owner orphans that still hold **data** are always skipped (they need a human), and `auto_purge_rgw()` is deliberately unchanged — it still touches only ledger-matched users. The empty-orphan count is recorded as `rgw_empty_unknown_owner` in the purge audit log; covered by `scripts/test_purge_orphans.py`
- **Full report (historical)** — `scripts/backfill_reports.sh` re-renders pre-upgrade run reports into the self-contained format. Before the renderer migration, each `<RUN_DIR>/<service>.html` was the old `rally task report` AngularJS page; under the new single strict CSP its CDN scripts (`ajax.googleapis.com`/`cdnjs.cloudflare.com`) are blocked, so any report from a run *before* the upgrade rendered as broken raw `{{...}}` placeholders. `entrypoint.sh` backgrounds the backfill (as the rally user, after the dashboard symlinks and before starting services) on every boot: it walks each `<RUN_DIR>` and re-renders `<service>.html` from the persisted `<service>.json` via `render_report.py`, so deploying this image heals historical reports immediately instead of waiting out `RALLY_RESULTS_RETENTION_DAYS`. Idempotent (skips a report already carrying the `id="rally-report-data"` marker) and best-effort (malformed/missing JSON → valid empty-state report). Its run-dir (`^[0-9]{8}T[0-9]{6}Z$`) and service-token (`^[a-z0-9_-]+$`) guards mirror `serve.py`'s `RUN_REPORT_RE` and `parse_rally_services`, and `RESULTS_DIR` is `%q`-quoted into the `su -c` invocation
