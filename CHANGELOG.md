...
- **Server** — `dashboard/serve.py` gains a fourth allowlist branch for served Rally HTML reports: `^runs/[0-9]{8}T[0-9]{6}Z/[a-z0-9_-]+\.html$` (anchored, no separators inside the captures) with re-checked `RESULTS_ROOT` containment on the resolved target, served via the new `/dashboard/runs -> /results` symlink. These reports get a **relaxed, per-path CSP** (`'unsafe-inline'` for script/style plus the two pinned CDN origins `ajax.googleapis.com` and `cdnjs.cloudflare.com` that rally 5.0.1's default html template references, plus a `sandbox` directive) applied to that branch **only**; every other response keeps the strict CSP, reset per request so a relaxed report response cannot leak its CSP onto a subsequent dashboard asset on a keep-alive connection. `ALLOWED_JSON_SYMLINKS` gains `run_state.json`
...
- **Tests** — the suite grew from 127 to 248 cases across this branch and is now gated by CI: `scripts/test_notify.py` (transition semantics, payload shapes, atomic state-on-success), `scripts/test_api_health_check.py` (single-session checker, degraded classification, `RALLY_SERVICES` parsing/subset, keystone-always), `scripts/test_run_summary.py` (`first_error` contract, `run_state`, seed-from-set), `exporter/test_rally_exporter.py` (API/announcement gauges), and `dashboard/test_serve.py` (the `runs/` report branch with denial cases and a cross-request CSP-leak test)
