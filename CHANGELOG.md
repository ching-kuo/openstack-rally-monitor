...
- **Tests** — the suite grew across this branch (now 270 passed + 1 skipped) and is now gated by CI: `scripts/test_notify.py` (transition semantics, payload shapes, atomic state-on-success), `scripts/test_api_health_check.py` (single-session checker, degraded classification, `RALLY_SERVICES` parsing/subset/allowlist, keystone-always), `scripts/test_run_summary.py` (`first_error` contract, `run_state`, seed-from-set), `exporter/test_rally_exporter.py` (API/announcement gauges, fail-closed `rally_api_overall_up`), `dashboard/test_getrunstatus.py` (failed-on-error/empty-services rendering), and `dashboard/test_serve.py` (the `runs/` report branch with denial cases and a cross-request CSP-leak test)
