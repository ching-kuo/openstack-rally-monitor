...
2. **Dashboard** (`dashboard/`) — Static files served by Python's `ThreadingHTTPServer` on `:8080`. The dashboard is pure HTML/JS/CSS with no build step; it fetches JSON from symlinked files (`results.json`, `history.json`, `health.json`, `health_history.json`, `announcement-state.json`, `run_state.json`) in the same directory, plus generated Rally HTML reports via the `runs/` symlink.
...
run_tests.sh
  ...
  → rally task results <uuid>          (JSON to /results/<timestamp>/<service>.json + <service>.html)
  ...
  (deployment_setup_failed path also runs record_smoke_result + notify.sh before exiting)

health_check.sh                        (thin cron wrapper: flock, atomic write, history append, all-down fallback)
...
`dashboard/serve.py` keeps a deny-by-default allowlist with four branches: `ALLOWED_STATIC` (own assets), `ALLOWED_JSON_SYMLINKS` (JSON symlinks resolving into `/results/`), the theme branch, and the served-Rally-report branch. Theme paths must live under `themes/`, use one of `.css`, `.svg`, `.png`, or `.ico`, and resolve either under `/dashboard/themes/` or under the tighter `/results/branding/` custom-theme root. Do not widen this to all of `/results/`.

The fourth branch serves generated Rally HTML reports: the regex `^runs/[0-9]{8}T[0-9]{6}Z/[a-z0-9_-]+\.html$` (anchored, no separators inside the captures) plus re-checked `RESULTS_ROOT` containment on the resolved target, reached via the `/dashboard/runs -> /results` symlink. These reports — and **only** these — get a relaxed, per-path CSP (`REPORT_SECURITY_HEADERS`): `'unsafe-inline'` for script/style plus the two CDN origins (`ajax.googleapis.com`, `cdnjs.cloudflare.com`) that rally 5.0.1's default html report template loads AngularJS/d3/nvd3 from, plus a `sandbox` directive. Every other response keeps the strict `SECURITY_HEADERS` (no `'unsafe-inline'` in `script-src`). `do_GET` resets `_security_headers` to strict at the top of every request so a report's relaxed CSP cannot leak onto a subsequent dashboard asset over a keep-alive connection (this is leak-tested). `ALLOWED_JSON_SYMLINKS` includes `run_state.json` and `announcement-state.json`, both of which 404 (dangling symlink, `target.exists()` false) until first written.
...
Located in `rally/scenarios/`. The monitored service set is configurable via `RALLY_SERVICES` (default `keystone,nova,neutron,glance,cinder,swift`), which drives `run_tests.sh`, `api_health_check.py`, and the `entrypoint.sh` seed (pending cards generated from the set). All three parse it with **identical normalization** — split on commas, trim/strip whitespace, lowercase, drop empty segments, dedupe preserving first-seen order — documented as **KEEP IN SYNC** between `run_tests.sh::parse_rally_services` and `api_health_check.py::parse_rally_services`. Unknown names are tolerated: a smoke run logs `SKIP` for a service with no `rally/scenarios/<name>.yaml`; the health check logs a warning and omits a name with no registered checker. `keystone` is always health-checked regardless (the openstacksdk session authenticates against it, so its token issuance *is* a keystone availability check; new scenario packs like octavia/heat/designate were deliberately deferred pending real-cloud validation). Scenarios are parameterized via `--task-args-file` with `RALLY_NOVA_FLAVOR`, `RALLY_NOVA_IMAGE`, and `OS_AUTH_URL` from the environment.
...
| `RALLY_SERVICES` | `keystone,nova,neutron,glance,cinder,swift` | Configurable monitored service set (normalized identically in `run_tests.sh`, `api_health_check.py`, and the entrypoint seed; keystone always health-checked) |
...
- API health gauges (from `health.json`): `rally_api_up{service}` and `rally_api_overall_up` (degraded counts as up — reachability, not speed; clear-before-set each scrape), `rally_api_latency_milliseconds{service}` (where degraded slowness is visible)
...
- Alert rules: copy `prometheus/rally_alerts.yml` to your Prometheus rules directory and add it under `rule_files:` in `prometheus.yml`. The `rally_api` group adds `RallyApiDown` (`rally_api_up == 0`, `for: 20m`); `RallyTestFailure`/`RallyServiceDown`/`RallySLABreach` carry commented opt-in `unless on() rally_maintenance_mode == 1` inhibition variants
